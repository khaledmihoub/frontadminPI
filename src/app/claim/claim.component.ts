...
export class ClaimComponent implements OnInit {
  ...
  send(){
    this.claimService.sendmail(this.sendmail.get('email').value,this.sendmail.get('message').value, this.jwt).subscribe(data=>{
     console.log("here?");
     this.currentClaim.status = "Completed";
     this.claimService.updateClaim( this.currentClaim,this.jwt).subscribe(res=> {
      console.log( res);
      this.ref = this.currentClaim.reference;
      this.variable= true;
      setTimeout(() => {
        this.variable = false;
      }, 8000);
  ...
  filteredClaims():any[] {
    ...
    return this.claims.filter((claim) =>
      claim.reference.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
      claim.status.toLowerCase().includes(this.searchTerm.toLowerCase())
    );
  }
